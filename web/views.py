...
def login(request):
    ...
    if request.method == 'GET':
        ...
        if int(request.GET.get('not_login', 0)):
...
def diyplan(request):
    # 先判断用户是否登录
    if not request.session.get('is_login', None):
        return redirect('../login?not_login=1')
    ...
    if request.method == 'GET':
        ...
        if s_id == '1':
            ...
            plan = [[1, "鼓浪屿-日光岩-菽庄花园-中山路步行街-八市场", [
                ...
                [1, '鼓浪屿 > 日光岩', '9:00-11:00', 50, '前往【日光岩】，这里是鼓浪屿的制高点，可以俯瞰鼓浪屿及厦门部分市区的景色', '日光岩.jpg'],
                [2, '那私厨｜红砖老别墅餐厅(鼓浪屿店)', 4.9, '闽菜', 100],
                [1, '鼓浪屿 > 菽庄花园', '13:00-14:00', 30, '参观【菽庄花园】，菽庄花园在鼓浪屿的地位等同于故宫在北京的地位，很是值得一去，园中还有四十四桥和钢琴博物馆，即可赏景又可听音',
                 '菽庄花园.jpg'],
                [1, '中山路步行街', '15:00-18:00', 0, '前往中国大陆仅有的一条直通大海的商业街【中山路步行街】自由逛吃，去主街附近的小巷中探寻历史厦门和地道人文，去找寻隐匿在小巷中的特色小吃',
                 '中山路步行街.jpg'],
                [1, '中山路步行街 > 八市', '15:00-16:00', 0,
                ...
                [2, '乌堂沙茶面(中山路店)', 4.6, '闽菜', 49]]],
                    ...
                    [3, "集美学村-龙舟池-鳌园-黄厝沙滩", [
                        [1, '集美学村', '9:00-12:00', 0, '参观【集美学村】临海而立，以华侨建筑群而闻名，这里学府聚集，伴随的是美食扎堆，既有浓厚的学识氛围，也有欢快的青年气息',
                         '集美学村.jpg'],
                        [1, '集美学村 > 龙舟池', '9:00-10:00', 0, '集美学村的一大亮点，由陈嘉庚先生督建，池畔的亭榭琉璃盖瓦、雕梁画栋，仿佛置身那个时代，奢享悠闲慢时光', '龙舟池.jpg'],
                        [2, '非常台非常泰', 4.7, '台湾菜', 70],
                        [1, '鳌园', '13:00-14:00', 0,
                ...
                while index_str < len(plan_str):
                    ...
                    for each in temp_list:
                        ...
                        if int(each[0]) == 1:
                            ...
                            left_index = right_index + 3
                            right_index = each.index("'", left_index)
                            if 'one' in each[left_index: right_index]:
